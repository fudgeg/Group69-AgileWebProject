...
def test_media_display_count(driver):
    ...
    # Go to For You page
    driver.get(f"{BASE}/foryou")
    WebDriverWait(driver, 5).until(EC.presence_of_element_located((By.CLASS_NAME, "media-card-small")))

    # Find the Books card and check the count
    media_cards = driver.find_elements(By.CLASS_NAME, "media-card-small")
    books_count_text = None
    for card in media_cards:
        if "Books" in card.text:
